class RequestManager {
  ...
  static apiBase = 'https://lessons-11.onrender.com/api/v1'
  ...
  static async doPostRequest(url,data,redirectRoute,callback,addAuthorization = true) {
    try {
      ...
      if (response.ok) {
        ...
        this.showErrors(resData.errors); // Використовуємо вже отримані дані resData
      }
    } catch (error) {
  ...
  static async deleteRequest(route, id, addAuthorization = true) {
    ...
    const response = await fetch(RequestManager.getServerRoute(route), {
      method: 'DELETE',
      headers: headers,
